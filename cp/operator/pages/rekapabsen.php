...
$idpeg=isset($_GET['pegid']) ? $_GET['pegid'] : '0';
		...
		<div class="col-lg-12 col-xs-12">
			...
			<div class="box">
				<div class="box-header">
				  <h3 class="box-title">
							<form class="form" action="rekapabsen.php" method="GET" id="tabID">
								<div class="row">
								...
								<div class="col-lg-6 col-xs-6">
										<div class="form-group">
											<select class="form-control" name="pegid" onchange="this.form.submit()">
												<option value="100" <?php if($nk['pegawai_id']===$idpeg){echo "selected";}; ?>>Pilih Pegawai</option>
												<?php 
												$sql_mk=mysqli_query($koneksi, "select * from id_pegawai order by pegawai_id asc");
												while($nk=mysqli_fetch_array($sql_mk)){
													$idnya=$nk['ptk_id'];
													$mm=mysqli_fetch_array(mysqli_query($koneksi, "select * from ptk where ptk_id='$idnya'"));
												?>
												<option value="<?=$nk['pegawai_id'];?>" <?php if($nk['pegawai_id']===$idpeg){echo "selected";}; ?>><?=$mm['nama'];?></option>
												<?php }; ?>
											</select>
										</div>
								</div>
				  ...
				  <div class="box-tools pull-right">
				    <a href="../../cetak/absenguru.php?bulanku=<?=$bln;?>&tahunku=<?=$thn;?>&tapel=<?=$tpl_aktif;?>" title="Absensi Bulan <?=$bulan[(int)$bln-1];?> <?=$thn;?>" class="btn" target="_blank">
						<i class="fa fa-print"></i> Rekap Absensi
					</a>
					<?php if($idpeg==0){
					...
					<a href="../../cetak/cetakslipgaji.php?idpeg=<?=$idpeg;?>&bln=<?=$bln;?>&thn=<?=$thn;?>" title="Cetak Slip Gaji Bulan <?=$bulan[(int)$bln-1];?> <?=$thn;?>" class="btn" target="_blank">
						<i class="fa fa-print"></i> Slip Gaji
					</a>
					<?php }; ?>
				  </div>
				</div>
				<div class="box-body table-responsive">
					...
					<i class="icon fa fa-check"></i> Telat : <?=$telat;?> Menit<br/>
...
<script>
	...
	$(document).ready(function() {
		...
		manageMemberTable = $("#manageMemberTable").DataTable({
			"ajax": "../modul/ptk/absensi.php?idpeg=<?=$idpeg;?>&bln=<?=$bln;?>&thn=<?=$thn;?>",
			"paging":false,
			"order": []
		});
	});
