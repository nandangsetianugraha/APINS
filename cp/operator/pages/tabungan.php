...
<script>
  ...
  function doneTyping() {
      $.post("../../tu/pages/ceknasabah.php",$("#tabID").serialize(),function (res) {
          $("span.loading").removeClass("fa fa-spinner fa-spin");
	...
	$(document).ready(function() {
		...
		viewTr();
		function viewTr(){
				$.get("../../tu/modul/tabungan/transaksi.php?tgl=<?=$sekarang;?>", function(data) {
					$("#transaksi").html(data);
				});
		}
		$('#idNasabah').on('keyup', function() {
			...
			$.ajax({
			  type: 'POST',
			  url: '../../tu/modul/tabungan/lihatsaldo.php',
			  data: {
				search: $(this).val()
			  },
			  cache: false,
		...
		manageMemberTable = $("#manageMemberTable").DataTable({
			"ajax": "../../tu/modul/tabungan/tabhariini.php?tanggal_now=<?=$sekarang;?>",
			"order": []
		});
		$('#datepicker').datepicker({
		  autoclose: true
		...
		$("#simpanTab").on('click', function() {
			...
			$("#tabID").unbind('submit').bind('submit', function() {
				...
				var form = $(this);
					...
					$.ajax({
						url : "../../tu/modul/tabungan/simpan.php",
	...
	function hapusTab(id = null) {
		if(id) {
			...
			$("#hapusBtn").unbind('click').bind('click', function() {
				$.ajax({
					url: '../../tu/modul/tabungan/hapusdata.php',
					type: 'post',
					data: {member_id : id},
					dataType: 'json',
					success:function(response) {
	...
	function hapusTr(id = null) {
		if(id) {
			...
			$("#hapusT").unbind('click').bind('click', function() {
				$.ajax({
					url: '../../tu/modul/tabungan/hapus.php',
					type: 'post',
					data: {member_id : id},
					dataType: 'json',
					success:function(response) {
