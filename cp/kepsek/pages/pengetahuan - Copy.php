...
if($level==98){ //guru bahasa inggris
	...
	$cekkkmp=mysqli_num_rows(mysqli_query($koneksi, "select * from kkmku where kelas='$ab' and tapel='$tapel' and mapel='$mpid'"));
	$cekkdp=mysqli_num_rows(mysqli_query($koneksi, "select * from kd where kelas='$ab' and aspek=3 and mapel='$mpid' group by kd"));
	$cekkdk=mysqli_num_rows(mysqli_query($koneksi, "select * from kd where kelas='$ab' and aspek=4 and mapel='$mpid' group by kd"));
	$jumkd=$cekkdp+$cekkdk;
	if($cekkkmp==$jumkd){
		$boleh=true;
	}else{
		...
		<div class="box box-primary">
			<div class="box-header">
              <h3 class="box-title"><?php 
								...
								?>
								Penilaian Aspek Pengetahuan <?=$mpm['nama_mapel'];?> Kelas <?=$romb;?> <?php if(($level==94 or $level==95 or $level==96) or ($ab>3 and $mpid==8) or ($ab>3 and $mpid==4) or ($ab>3 and $mpid==9) or ($ab>3 and $mpid==10) or ($ab>3 and $mpid==11)){ ?> Pembelajaran <?php }else{ ?> Tema <?php }; ?> <?=$tema;?></h3>
			  <div class="box-tools pull-right">
				...
				<div class="form-group">
					...
					<input type="hidden" name="peta" class="form-control" value="3">
					<?php 
											$tma1=mysqli_query($koneksi, "select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='$peta' and mapel='$mpid' and tema='$tema' order by nama_peta asc");
			...
			<div class="box-body">
				<div class="row">
					<div class="col-lg-12 col-xs-12 table-responsive no-padding">
						<?php 
											if(!empty($kd)){
												if($boleh){
						?>
							...
							<div class="callout callout-success">
								<h4>Belum Mengisi KKM <?=$mpm['nama_mapel'];?>, silahkan isi terlebih dahulu dan lengkapi KKM <?=$mpm['nama_mapel'];?>.</h4>
							</div>					
							<?php
												}}else{ ?>
							<div class="callout callout-success">
