...
*/
 ...
 while($sis=$qkl->fetch_assoc()){
	 ...
	 $qkl1 = $connect->query($skl1);
		 ...
		 $table3->easyCell($namakd['rk'],'align:C;');
		 for ($i = 5; $i < 9; $i++){
		 ...
		 $table3->easyCell(number_format($rNH['nilai_nh'],0),'align:C;');
		 ...
		 $table3->easyCell(number_format($nPTS['nilai'],0),'align:C;');
		 ...
		 $table3->easyCell(number_format($nPAS['nilai'],0),'align:C;');
		 if($nPTS['nilai']==0){
			 $naKD=(2*$rNH['nilai_nh']+$nPAS['nilai'])/3;
		 }else{
			 $naKD=(2*$rNH['nilai_nh']+$nPAS['nilai']+$nPTS['nilai'])/4;
		 };
		...
		while($sis1=$qkl1->fetch_assoc()){
		 ...
		 $table3->easyCell($namakd1['rk'],'align:C;');
		 for ($i = 5; $i < 9; $i++){
