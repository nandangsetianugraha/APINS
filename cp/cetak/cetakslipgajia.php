	...
	$bulan=array("","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember");
		...
		for ($i=1; $i < $hari+1; $i++) { 
			...
			if(namahari($ttt)==="Sabtu" || namahari($ttt)==="Minggu"){
				...
				$cek=$connect->query("select * from absensi_ptk where pegawai_id='$idpeg' and DATE_FORMAT(tanggal,'%Y-%m-%d')='$ttt'");
				$hab = $connect->query("SELECT pegawai_id,
				...
				FROM absensi_ptk where pegawai_id='$idpeg' and DATE_FORMAT(tanggal,'%Y-%m-%d') = '$ttt'");
				$ada=$cek->num_rows;
				if($ada>0){
